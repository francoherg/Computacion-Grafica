...
int Delaunay::enQueTriangulo(glm::vec3 &punto) const {
	int i_tri = rand()%triangulos.size();//elije un triangulo cualquiera en el delaunay
	while (i_tri!=-1) { //si el indice es -1 entonces el punto esta fuera de la interpolacion y no se debe hacer nada
		...
		}//encuentra el valor mas negativo de los pesos, para ir por ese camino (por el triangulo en el lado opuesto al vertice)
		if (ff[imin]>=0) break; //si el peso dentro del triangulo es todo positivo, entonces es ese el triangulo donde se encuentra el punto
		i_tri = triangulos[i_tri].vecinos[imin];//devuelve el indice de delaunay del triangulo
	}
	return i_tri;
}
