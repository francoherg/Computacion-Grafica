...
// settings
std::vector<std::string> models_names = { "suzanne", "fish", "chookity" };
int current_model = 0;
...
glm::vec3 warpPoint(const Delaunay &delaunay0, const Delaunay &delaunay1, glm::vec3 p) {
	...
	const std::vector<glm::vec3> &v_puntos_delaunay0= delaunay0.getPuntos();//como estan los puntos al principio
	
	const std::vector<glm::vec3> &v_p1= delaunay1.getPuntos();//como estan los puntos despues de mover un punto
	
	
	
	const std::vector<Triangulo> &triangles = delaunay0.getTriangulos();
	Triangulo t0= triangles[delaunay0.enQueTriangulo(p)]; // en que triangulo de delaunay0 esta el punto p 
	
	auto t0_vertices=t0.vertices; // los vertices de ese triangulo (indices en delaunay0)
	
	auto v0=v_puntos_delaunay0[t0_vertices[0]]; //consigo las posiciones de estos vertices
	auto v1=v_puntos_delaunay0[t0_vertices[1]];
	auto v2=v_puntos_delaunay0[t0_vertices[2]];
	
	
	auto pesos_orig = calcularPesos(v0,v1,v2,p); //calculo los pesos usando esos vertices y el punto p
	
	
	glm::vec3 vertice0=v_p1[t0_vertices[0]]; //los mismos vertices pero en delaunay1, para notar como se movieron
	glm::vec3 vertice1=v_p1[t0_vertices[1]];
	glm::vec3 vertice2=v_p1[t0_vertices[2]];
	
	
	auto p_z=p[2];//guardo el valor de z
	
	p = pesos_orig[0] * vertice0 + pesos_orig[1] * vertice1 +pesos_orig[2] * vertice2; 

	//^^el nuevo valor de p es el de la interpolacion lineal afin de los vertices nuevos 
	//pero manteniendo el peso original en lugar de utilizar calcularPesos con los vertices nuevos
	
	p= {p.x, p.y, p_z};//mantengo el z original
